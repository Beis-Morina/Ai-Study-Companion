...
document.getElementById('auth-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        alert("Serveri nuk po pergjigjet.");
    }
});
...
async function handleGenerate() {
    if (!authToken) { location.reload(); return; }
    ...
    try {
        const response = await fetch(`${API_BASE}/api/chat/send`, {
            ...
            body: JSON.stringify({ message: `Tema: ${topic}. Detaje: ${desc}`, chat_id: currentChatId })
        });
        const data = await response.json();
        if (response.ok) {
            if (data.chat_id) { currentChatId = data.chat_id; fetchConversations(); }
    ...
    } catch (err) { alert("Gabim gjate lidhjes me AI."); }
}
